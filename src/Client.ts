namespace Netcode {
  ...
  export class Client {
    ...
    public constructor(token: ConnectToken) {
      ...
      this._payloadPacketQueue = new Queue<IPacket>(PACKET_QUEUE_SIZE);
    ...
    public tick(t: number) {
      ...
      if (state > ClientState.disconnected && state < ClientState.connected) {
        ...
        if (this._startTime + expire <= this._time) {
          this.debugLog(
            `client${this.id} connect failed. connect token expired`
          );
    ...
    private onPacketData(packetData: Uint8Array, from: IUDPAddr) {
